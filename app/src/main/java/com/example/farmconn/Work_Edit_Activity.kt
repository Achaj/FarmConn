...
class Work_Edit_Activity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (machineArrayList != null) {
            val myAdapter = MyListAdapterViev_MachineAtWork__EDIT(
                this,
        ...
        if (fieldsArrayList != null) {
            val myAdapter = MyListAdapterView_FieldAtWork__EDIT(
                this,
                R.layout.list_fields_at_work,
                fieldsArrayList
            )
        ...
        if (userArrayList != null) {
            val myAdapter =
                MyListAdapterView_UserAtWork_EDIT(this, R.layout.item_user_at_work, userArrayList)
            ...
            builder.setPositiveButton(android.R.string.ok) { dialog, which ->
                if (db.delateWorkMachineByIdWork(workOLD.idWork) > -1) {
                    if (db.delateWork(workOLD) > -1) {
                        ...
                        Toast.makeText(
                            this,
                            "Została usunięta paraco o id " + workOLD.idWork.toString(),
                            Toast.LENGTH_LONG
                        ).show()
                    }
                }
        ...
        saveWork_Button_AWE.setOnClickListener {
            ...
            builder.setPositiveButton(android.R.string.ok) { dialog, which ->
                if (db.updateWork(workOLD) > -1) {
                    ...
                    Toast.makeText(
                        applicationContext,
                        "Dane zostały zaktualizowanie",
                        Toast.LENGTH_SHORT
                    ).show()
                } else {
                    Toast.makeText(applicationContext, "Coś poszło nie tak", Toast.LENGTH_SHORT)
                        .show()
                }
            }
            builder.setNegativeButton(android.R.string.cancel) { dialog, which ->
