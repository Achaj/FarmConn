...
class FarmActivity : AppCompatActivity() {
           ...
           if (farmArrayList != null) {
            ...
            edit_Button_AF.setOnClickListener{
                ...
                try{
                    ...
                    Toast.makeText(applicationContext, "Błąd konwersji zamień , na .", Toast.LENGTH_LONG).show()
                }
                var yC:String?=null
            ...
            showFieldInMapBTN.setOnClickListener{
                val uri: String = java.lang.String.format("geo:<" + farm!!.xCord + ">,<" + farm!!.yCord + ">?q=<" + farm!!.xCord  + ">,<" + farm!!.yCord + ">" +
                        "(" + farm!!.nameFarm + ")")
               ...
               var user=helperUser.getCurrentUser()
                if (user != null) {
                    ...
                    if(dbHandler.updateUser(user)>-1){
                        Toast.makeText(applicationContext, "Twoje nowa farma to  " + farm!!.nameFarm, Toast.LENGTH_LONG)
                            .show()
                        farm=null
                        tmpFarm=null
                        val intent= Intent(this,FarmActivity::class.java)
    ...
    fun putData(farm: Farm){
        Log.d("FARM","--"+farm.idFarm +"-"+ farm.nameFarm + farm.xCord.toString() + farm.yCord.toString())
        findViewById<EditText>(R.id.name_EditText_AF).text= Editable.Factory.getInstance().newEditable(farm.nameFarm)
        findViewById<EditText>(R.id.x_EditText_AF).text= Editable.Factory.getInstance().newEditable(farm.xCord.toString())
        findViewById<EditText>(R.id.y_EditText_AF).text= Editable.Factory.getInstance().newEditable(farm.yCord.toString())
    }
