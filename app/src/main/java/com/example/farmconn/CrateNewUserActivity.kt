            ...
            if (name != "null" && surname != "null" && email != "null" && pass != "null" && passConfirm != "null") {
                ...
                if (okName == true && okSurname == true && okEmail == true && okPass == true) {
                    ...
                    if (okName != true) {
                        Toast.makeText(
                            applicationContext,
                            "Twoje imie może się skaładać tylko z liter",
                            Toast.LENGTH_LONG
                        ).show()
                    }
                    if (okSurname != true) {
                        Toast.makeText(
                            applicationContext,
                            "Twoje nazwisko może się skaładać tylko z liter",
                            Toast.LENGTH_LONG
                        ).show()
                    }
                    if (okEmail != true) {
                        Toast.makeText(
                            applicationContext,
                            "Twój email nie jest w odpowiedmin foramacie xyz@xyx.xyz",
                            Toast.LENGTH_LONG
                        ).show()
                    }
                    if (okPass != true) {
                        Toast.makeText(
                            applicationContext,
                            "Hasło musi się składać z liter i cyfr o długości minimu 8 znaków hasła muszą być identyczne",
                            Toast.LENGTH_LONG
                        ).show()
                    }
