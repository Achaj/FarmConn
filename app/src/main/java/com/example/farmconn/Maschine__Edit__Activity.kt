...
class Maschine__Edit__Activity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        showFieldInMapBTN.setOnClickListener {
            val uri: String =
                java.lang.String.format("geo:<" + maschineOLD.xCords + ">,<" + maschineOLD.yCords + ">?q=<" + maschineOLD.xCords + ">,<" + maschineOLD.yCords + ">(" + maschineOLD.typeMachine + ")")
    ...
    private fun putData(machine: Machine?) {
        ...
        if (machine != null) {
            findViewById<EditText>(R.id.brand_editText_AME).text =
                Editable.Factory.getInstance().newEditable(machine.brandMachine)
            findViewById<EditText>(R.id.model_editText_AME).text =
                Editable.Factory.getInstance().newEditable(machine.modelMachine)
            findViewById<EditText>(R.id.type_editText_AME).text =
                Editable.Factory.getInstance().newEditable(machine.typeMachine)
            findViewById<EditText>(R.id.capacity_editText_AME).text =
                Editable.Factory.getInstance().newEditable(machine.capacityMachine.toString())
            findViewById<EditText>(R.id.fuel_editText_AME).text =
                Editable.Factory.getInstance().newEditable(machine.fuelUsageMachine.toString())
            findViewById<EditText>(R.id.weight_editText_AME).text =
                Editable.Factory.getInstance().newEditable(machine.weightMachine.toString())
            findViewById<EditText>(R.id.width_editText_AME).text =
                Editable.Factory.getInstance().newEditable(machine.widthMachine.toString())
        }
    }
}